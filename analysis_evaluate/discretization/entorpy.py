...
def calc_entropy2(count):
    ...
    entropy = [entropy_clockwise.values[idx] + entropy_anticlockwise.values[len(entropy_clockwise.index) - 2 - idx] for
               idx in range(len(entropy_clockwise.index) - 1)]
    return entropy
...
if __name__ == '__main__':
    ...
    df = e_data.get_iris2()
    group = Entropy_Discretization(df, 'sepal_width', 'y', max_interval=4, binning_method='entropy', feature_type=0)
    print(group)
